...
![](https://latex.codecogs.com/gif.latex?f(x)=\sum_{i=1}^{n}\alpha_{i}\chi_{E_i}(x))   
其中，E<sub>i</sub>是可测集合并满足   
![](https://latex.codecogs.com/gif.latex?\bigcup_{i=1}^{n}E_{i}=E\text{,%20}E_{i}\cap%20E_{j}=\varnothing\text{%20}(i\neq%20j))   
![](https://latex.codecogs.com/gif.latex?\chi_{E_i}(x))
...
![](https://latex.codecogs.com/gif.latex?A=y_{0}<y_{1}<\dots<y_{n}=B)   
令![](https://latex.codecogs.com/gif.latex?e_k=\\{x:x\in%20E%20\text{%20and%20}y_{k}\le%20f(x)<y_{k+1}\\})  
则Lebesgue小和s和大和S分别定义为    
![](https://latex.codecogs.com/gif.latex?s=\sum_{k=0}^{n-1}y_{k}\mu(e_{k})\text{,}S=\sum_{k=0}^{n-1}y_{k+1}\mu(e_{k}))    
其中μ(e<sub>k</sub>)表示可测集合e<sub>k</sub>的测度。


**[定义4 可测函数的积分的定义1]**   
设f(x)为可测函数,s和S分别为其Lebesgue小和和大和，令   
![](https://latex.codecogs.com/gif.latex?U=sup\\{s\\}\text{,%20}V=inf\\{S\\})   
当![](https://latex.codecogs.com/gif.latex?\lambda=max(y_{i+1}-y_{k})\rightarrow0) 时，如果U=V，则称f(x)在E上可积，记为   
![](https://latex.codecogs.com/gif.latex?\int_{E}f(x)dx=U(V)) 
...
![](https://latex.codecogs.com/gif.latex?\int|f_n-f_m|dx\rightarrow0\text{%20when%20}m,n\rightarrow0)    
则称之为依平均的Cauchy序列
...
![](https://latex.codecogs.com/gif.latex?\lim_{n\to+\infty}\int%20h_n(x)dx=\lim_{n\to+\infty}\int%20g_n(x)dx) 
...
设f(x)为广义实值可测函数，如果存在一个可积简单函数序列{f<sub>n</sub>}满足如下性质   
![](https://latex.codecogs.com/gif.latex?\text{(a)}\{f_n\}\text{%20is%20a%20Cauchy%20sequence%20in%20the%20mean})   
![](https://latex.codecogs.com/gif.latex?\text{(b')}{f_{n}}\text{%20converges%20in%20measure%20to%20}f(x))    
则称f(x)可积
...
![](https://latex.codecogs.com/gif.latex?\\int_{E}f(x)dx=\sup_{h(x)<=f(x)}\left\\{\int_{E}h(x):h(x)\text{%20is%20a%20simple%20function}\right\\}) 
